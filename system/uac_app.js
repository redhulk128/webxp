...
<div style="width:100%;height:100%;background-color:#6375d6;" class="outercontent">
  <div style="margin-left:32px;width:calc(100% - 64px);" class="content">
    ...
    function changeAccount(acct) {
      ...
      <p>
        <span style="color:#000;" class="cp_option pointer change_name">Change ${
          acct.name === xp.profile.name ? "my" : "the"
        } name</span>
      </p>
      <p>
        <span style="color:#000;" class="cp_option pointer change_picture">Change ${
          acct.name === xp.profile.name ? "my" : "the"
        } picture</span>
      </p>${canDelete}
      <table style="font-size:11px;color:#000;position:absolute;left:269px;top:97px">
        <tr><td rowspan="4" style="width:64px;"><img class="userimg" style="width:48px;height:48px;" src="${
          acct.image
        }"/></td></tr>
    ...
    function deleteAccount(acct) {
      ...
      win.el.find(".keep").on("click", () => {
        xp.filesystem.moveDir(
          "/Documents and Settings/" + acct.name + "/My Documents",
          "/Documents and Settings/" +
            xp.profile.name +
            "/My Documents/" +
            acct.name,
          () => {
            xp.filesystem.deleteDir(
              "/Documents and Settings/" + acct.name,
              () => {
                win.close();
              }
            );
      ...
      win.el.find(".change").on("click", () => {
        ...
        if (xp.profile.name !== newUsername) {
          xp.filesystem.moveDir(
            ...
            `/Documents and Settings/${newUsername}`,
            () => {}
          );
        ...
        saveConfig(() => {
          ...
          xp.alert(
            "Changes have been applied.<br/>Click OK to reboot your computer.",
            () => {
    ...
    function changePicture(acct) {
      ...
      <p style="margin-left:16px;overflow-x:scroll;width:100%;">
        <table style="font-size:11px;color:#000;">
          ...
          <td><img class="userimgopt" src="https://i.postimg.cc/qqnzbNzs/01d2e04f-e49d-4304-aa9e-55b9849b4cce-usertile9.png"/></td>
          <td><img class="userimgopt" src="https://i.postimg.cc/s2vfHMLM/01d2e04f-e49d-4304-aa9e-55b9849b4cce-usertile10.png"/></td>
          <td><img class="userimgopt" src="https://i.postimg.cc/TPLGqZym/01d2e04f-e49d-4304-aa9e-55b9849b4cce-usertile11.png"/></td>
          <td><img class="userimgopt" src="https://i.postimg.cc/zBTr4KFG/01d2e04f-e49d-4304-aa9e-55b9849b4cce-usertile12.png"/></td>
          </tr>
        </table>
      </p>
      <p>
      ...
      if (win.el.find(`.userimgopt[src="${acct.image}"]`).length > 0)
        ...
        win.el
          .find(".images")
          .append(
            `<td><img class="userimgopt selected" src="${acct.image}"/></td>`
          );
      ...
      win.el.find(".browse").on("click", () => {
        xp.filesystem.openFileDialog((file) => {
          xp.filesystem.toURL(file, (url) => {
            ...
            var el = $(
              $.parseHTML(
                `<td><img class="userimgopt selected" src="${url}"/></td>`
              )
            );
    ...
    function listAccounts() {
      xp.filesystem.listDir("/Documents and Settings", (acct) => {
        acct = acct.split("/")[0];
        xp.filesystem.readFile(
          `/Documents and Settings/${acct}/config.json`,
          (file) => {
    ...
    <div style="text-align:center;" class="pointer">${acct}</div>
            ...
            $(el)
              .find("img")
              .on("click", () => changeAccount(config.profile));
    ...
    <div style="position:absolute;right:8px;bottom:8px;">
      ...
      win.el.find(".next").on("click", function () {
        if (win.el.find(".username").val() === "") {
          ...
          xp.wallpaper.href =
            "https://cdn.glitch.com/01d2e04f-e49d-4304-aa9e-55b9849b4cce%2FBliss.jpg?1519950052202";
          win.content(`
<div style="margin-left:32px;">
