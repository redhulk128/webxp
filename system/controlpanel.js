...
function loadConfig(callback) {
  ...
  xp.filesystem.readFile(configFile, (text) => {
    try {
      ...
      xp.wallpaper.set(config.wallpaper || 'https://redhulk.in/webxp/themes/XP.jpg');
      xp.profile = config.profile;
      xp.theme.set(config.theme || 'webxp');
    } catch (e) {
      xp.wallpaper.set('https://redhulk.in/webxp/system/themes/XP.jpg');
      loadingconfig = false;
      saveConfig();
    }
    loadingconfig = false;
...
xp.controlpanel.add('Themes', () => {
  ...
  win.el.find('#changeWallpaperBtn').on('click', function() {
    ...
    const wallpaperThumbnails = [
      ...
      'system/themes/wallpapers/Bird.jpg',
      'system/themes/wallpapers/ai.jpg',
    ];
